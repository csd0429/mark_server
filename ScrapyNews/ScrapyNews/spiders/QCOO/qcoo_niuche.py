...
class ExampleSpider(CrawlSpider):
    name = "qcoo_niuche"
    allowed_domains = ["niuche.com"]
    start_urls = ['http://www.niuche.com/ask/jinghua',]
    rules=(
        Rule(LinkExtractor(allow=r"/ask/e_\d{6}.html"),
        callback="parse_news",follow=True),
    )
    for i in range(1,10):
        start_urls.append('http://www.niuche.com/ask/jinghua?p='+str(i))
    ...
    def get_news_id(self,response,item):
        item['news_id']='NC'+response.url[-11:-5]

    def get_news_title(self,response,item):
        news_title=response.xpath("/html/body/div[3]/div[1]/div[1]/h1/text()").extract()
        if news_title:
            item['news_title']=news_title[0]

    def get_news_info(self,response,item):
        news_info=response.xpath("//*[@id='recwContent']/p/descendant::text()").extract()
        if news_info:
    ...
    def get_news_img(self,response,item):
        news_img=response.xpath("//*[@id='recwContent']/p/descendant::img/@src").extract()
        if news_img:
            result='{'
            count=0
            for img in news_img:
    ...
    def get_news_date(self,response,item):
        news_date=response.xpath("/html/body/div[3]/div[1]/div[1]/div[1]/div/div/p/text()").extract()
        if news_date:
            newsdate=datetime.datetime.strptime('20'+str(news_date[0]),"%Y-%m-%d %H:%M")
