...
class ExampleSpider(CrawlSpider):
    ...
    start_urls = ['http://www.dailyvitamin.cn/',]
    rules=(
        Rule(LinkExtractor(allow=r"/food/\d{8}/\d{3}.html"),
        callback="parse_news",follow=True),
    )
    for i in range(1,10):
        start_urls.append('http://www.dailyvitamin.cn/food?page='+str(i))
    ...
    def get_news_info(self,response,item):
        ...
        news_info=response.xpath("//*[@id='node-"+response.url[-8:-5]+"']/div[2]/div[1]/div/div/p/descendant::text()").extract()
        if news_info:
