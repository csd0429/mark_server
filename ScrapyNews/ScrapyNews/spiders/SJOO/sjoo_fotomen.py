...
class ExampleSpider(CrawlSpider):
    ...
    start_urls = ['http://fotomen.cn/category/subject/',]
    rules=(
        Rule(LinkExtractor(allow=r"/\d{4}/\d{2}/\w+"),
        callback="parse_news",follow=True),
    )
    for i in range(1,5):
        start_urls.append('http://fotomen.cn/category/subject/page/'+str(i)+'/')
    ...
    def get_news_id(self,response,item):
        item['news_id']='FT00'+response.url[-5:-1]

    def get_news_title(self,response,item):
        news_title=response.xpath("//*[@id='cb-full-width-featured']/div[1]/span/h1/text()").extract()
        print news_title
        if news_title:
            item['news_title']=news_title[0]

    def get_news_info(self,response,item):
        news_info=response.xpath("//*[@id='main']/article/section/p/descendant::text()").extract()
        if news_info:
    ...
    def get_news_img(self,response,item):
        news_img=response.xpath("//*[@id='main']/article/section/p/descendant::img/@src").extract()
        if news_img:
            result='{'
            count=0
            for img in news_img:
    ...
    def get_news_date(self,response,item):
        news_date=response.xpath("//*[@id='cb-full-width-featured']/div[1]/span/div/div[2]/time/@datetime").extract()
        if news_date:
