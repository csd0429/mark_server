...
class ExampleSpider(CrawlSpider):
    ...
    start_urls = ['http://kuailiyu.cyzone.cn/',]
    rules=(
        Rule(LinkExtractor(allow=r"/article/\d{5}.html"),
        callback="parse_news",follow=True),
    )
    for i in range(1,10):
        start_urls.append('http://kuailiyu.cyzone.cn/index_'+str(i)+'.html')
    ...
    def get_news_id(self,response,item):
        ...
        item['news_id']='KLY'+response.url[-10:-5]

    def get_news_title(self,response,item):
        news_title=response.xpath("//*[@id='art_content']/h1/text()").extract()
        if news_title:
    ...
    def get_news_date(self,response,item):
        news_date=response.xpath("//*[@id='art_content']/div[1]/span[1]/text()").extract()
        if news_date:
            newsdate=datetime.datetime.strptime(str(news_date[0][5:]),"%Y-%m-%d %H:%M:%S")
