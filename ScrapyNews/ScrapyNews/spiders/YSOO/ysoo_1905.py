...
class ExampleSpider(CrawlSpider):
    ...
    rules=(
        Rule(LinkExtractor(allow=r"/news/\d{8}/\d{6}.shtml"),
        callback="parse_news",follow=True),
    )
    for i in range(1,10):
        start_urls.append('http://www.1905.com/list-p-catid-220.html?page='+str(i))
    ...
    def get_news_id(self,response,item):
        item['news_id']='19'+response.url[-12:-6]


    def get_news_title(self,response,item):
        news_title=response.xpath("//*[@id='contentNews']/h1/text()").extract()
        if news_title:
            item['news_title']=news_title[0]

    def get_news_info(self,response,item):
        news_info=response.xpath("//*[@id='contentNews']/div[3]/p/descendant::text()").extract()
        if news_info:
    ...
    def get_news_img(self,response,item):
        news_img=response.xpath("//*[@id='contentNews']/div[3]/p/descendant::img/@src").extract()
        if news_img:
            result='{'
            count=0
            for img in news_img:
    ...
    def get_news_date(self,response,item):
        news_date=response.xpath("//*[@id='contentNews']/div[1]/span[1]/text()").extract()
        if news_date:
            ...
            daterange=today-datetime.timedelta(days=10)
