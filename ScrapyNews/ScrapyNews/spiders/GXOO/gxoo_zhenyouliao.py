...
class ExampleSpider(CrawlSpider):
    name = "gxoo_zhenyouliao"
    allowed_domains = ["zhenyouliao.com"]
    start_urls = ['http://www.zhenyouliao.com/new/',]
    rules=(
        Rule(LinkExtractor(allow=r"/\w+/\d{5}"),
        callback="parse_news",follow=True),
    )
    for i in range(1,10):
        start_urls.append('http://www.zhenyouliao.com/new/'+str(i)+'/')
    ...
    def get_news_id(self,response,item):
        ...
        item['news_id']='ZYL'+response.url[-5:]

    def get_news_title(self,response,item):
        news_title=response.xpath("//*[@id='t']/text()").extract()
        if news_title:
            item['news_title']=news_title[0]

    def get_news_info(self,response,item):
        news_info=response.xpath("//*[@id='p-page']/div[2]/div/div[1]/div[1]/div[1]/div[2]/p/descendant::text()").extract()
        if news_info:
    ...
    def get_news_img(self,response,item):
        news_img=response.xpath("//*[@id='p-page']/div[2]/descendant::img/@src").extract()
        if news_img:
            result='{'
            count=0
            for img in news_img:
    ...
    def get_news_date(self,response,item):
        news_date=response.xpath("//*[@id='p-page']/div[2]/div/div[1]/div[1]/div[1]/p/span[2]/text()").extract()
        if news_date:
            newsdate=datetime.datetime.strptime(news_date[0].encode("utf-8").replace('年','-').replace('月','-')
             .replace('日','')+':'+str(random.randint(0,59)),"%Y-%m-%d %H:%M:%S")
