...
class ExampleSpider(CrawlSpider):
    name = "gxoo_cri"
    allowed_domains = ["news.cri.cn"]
    start_urls = ['http://i.cri.cn/popularnews/',]
    rules=(
        Rule(LinkExtractor(allow=r"/\d{4}-\d+-\d+/*"),
    ...
    def get_news_id(self,response,item):
        item['news_id']='CR'+response.url[-12:-6]

    def get_news_title(self,response,item):
        news_title=response.xpath("//*[@id='atitle']/text()").extract()
        if news_title:
            item['news_title']=news_title[0]

    def get_news_info(self,response,item):

        news_info=response.xpath("//*[@id='abody']/p/descendant::text()").extract()
        if news_info:
    ...
    def get_news_img(self,response,item):
        news_img=response.xpath("//*[@id='abody']/p/descendant::img/@src").extract()
    ...
    def get_news_date(self,response,item):
        news_date=response.xpath("//*[@id='acreatedtime']/text()").extract()
        if news_date:
