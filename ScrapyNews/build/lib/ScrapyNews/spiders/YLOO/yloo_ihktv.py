...
class ExampleSpider(CrawlSpider):
    name = "ihktv"
    allowed_domains = ["www.ihktv.com"]
    start_urls = ['http://www.ihktv.com/',]
    rules=(
    ...
    def get_news_title(self,response,item):
        news_title=response.xpath("//*[@id='main']/article/h3/text()").extract()
        if news_title:
            item['news_title']=news_title[0]

    def get_news_info(self,response,item):

        news_info=response.xpath("//*[@id='main']/article/div[1]/p/descendant::text()").extract()
    ...
    def get_news_img(self,response,item):
        news_img=response.xpath("//*[@id='main']/article/div[1]/descendant::img/@src").extract()
    ...
    def get_news_date(self,response,item):
        news_date=response.xpath("//*[@id='main']/article/header/time/@datetime").extract()
        if news_date:
            item['news_date']=news_date[0]

    def get_news_source(self,response,item):
        item['news_source']='香港娱乐频道'
