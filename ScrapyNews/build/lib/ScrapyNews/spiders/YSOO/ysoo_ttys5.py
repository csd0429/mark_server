...
class ExampleSpider(CrawlSpider):
    name = "ttys5"
    allowed_domains = ["m.ttys5.com"]
    start_urls = ['http://m.ttys5.com/',]
    rules=(
        Rule(LinkExtractor(allow=r"/show.php\?classid=\d+&id=\d+&style=\d+&cpage=\d+&cid=\d+&bclassid=\d+"),
    ...
    def get_news_title(self,response,item):
        news_title=response.xpath("/html/body/div[5]/h1/text()").extract()
        if news_title:
