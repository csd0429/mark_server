...
class ExampleSpider(CrawlSpider):
    name = "weart"
    allowed_domains = ["weartweart.com"]
    start_urls = ['http://weartweart.com/',]
    rules=(
    ...
    def get_news_id(self,response,item):
        ...
        item['news_id']='WT'+id+response.url[-5:]

    def get_news_title(self,response,item):
        news_title=response.xpath("//*[@id='main_content']/div[1]/header/h1/text()").extract()
        if news_title:
            item['news_title']=news_title[0][14:-10]

    def get_news_info(self,response,item):

        news_info=response.xpath("//*[@id='main_content']/div[2]/p/descendant::text()").extract()
    ...
    def get_news_img(self,response,item):
        news_img=response.xpath("//*[@id='main_content']/div[2]/p/descendant::img/@src").extract()

        if news_img:
            item['news_img']=news_img

    def get_news_date(self,response,item):
        news_date=response.xpath("//*[@id='main_content']/div[1]/header/div/time/@datetime").extract()
        if news_date:
            item['news_date']=news_date[0]

    def get_news_source(self,response,item):
        item['news_source']='weart'
